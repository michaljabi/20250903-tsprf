...
function userService() {
  const userFactory = ( name: string, age: number, profession: string ): Professional => ({ name, age, profession });
...
*/
 ...
 const makeLi = (user: Professional) => li(`${user.name} (${user.age}) - ${user.profession}`);
